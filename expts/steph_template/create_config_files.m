function create_config_files(varargin)
    ...
    parser.addParamValue('experiment_name', 'UNNAMED_EXPT', @ischar); 
    parser.addParamValue('email_for_notifications', '1live.life.queen.size1@gmail.com', @ischar); 
    parser.addParamValue('yeti_user', 'et2495', @ischar); 
    ...
    if(strcmp( structure_type, 'loopy') == 1)
      fprintf(fid,'#PBS -l nodes=1:ppn=2,walltime=119:00:00,mem=15000mb\n');
    else
      fprintf(fid,'#PBS -l nodes=1:ppn=1,walltime=71:00:00,mem=8000mb\n');
    end
    ...
    fprintf(fid,'#PBS -o localhost:/vega/brain/users/%s/src/crf/fwMatch/expt/%s/yeti_logs/\n', yeti_user, experiment_name);
    fprintf(fid,'#PBS -e localhost:/vega/brain/users/%s/src/crf/fwMatch/expt/%s/yeti_logs/\n', yeti_user, experiment_name);
