function [prec, tpr, fpr, thresh] = prec_rec(score, target, ...
                                             varargin)
% PREC_REC - Compute and plot precision/recall and ROC curves.
%
%   PREC_REC(SCORE,TARGET), where SCORE and TARGET are equal-sized
%   vectors,
%   and TARGET is binary, plots the corresponding precision-recall
%   graph
%   and the ROC curve.
%
%   Several options of the form PREC_REC(...,'OPTION_NAME',
%   OPTION_VALUE)
%   can be used to modify the default behavior.
%      - 'instanceCount': Usually it is assumed that one line in
%      the input
%                         data corresponds to a single
%                         sample. However, it
%                         might be the case that there are a total
%                         of N
%                         instances with the same SCORE, out of
%                         which
%                         TARGET are classified as positive, and (N
%                         -
%                         TARGET) are classified as
%                         negative. Instead of
%                         using repeated samples with the same
%                         SCORE, we
%                         can summarize these observations by means
%                         of this
%                         option. Thus it requires a vector of the
%                         same
%                         size as TARGET.
%      - 'numThresh'    : Specify the (maximum) number of score
%      intervals.
%                         Generally, splits are made such that each
%                         interval contains about the same number
%                         of sample
%                         lines.
%      - 'holdFigure'   : [0,1] draw into the current figure,
%      instead of
%                         creating a new one.
%      - 'style'        : Style specification for plot command.
%      - 'plotROC'      : [0,1] Explicitly specify if ROC curve
%      should be
%                         plotted.
%      - 'plotPR'       : [0,1] Explicitly specify if
%      precision-recall curve
%                         should be plotted.
%      - 'plotBaseline' : [0,1] Plot a baseline of the random
%      classifier.
%
%   By default, when output arguments are specified, as in
%         [PREC, TPR, FPR, THRESH] = PREC_REC(...),
%   no plot is generated. The arguments are the score thresholds,
%   along
%   with the respective precisions, true-positive, and
%   false-positive
%   rates.
%
%   Example:
...
if (plot_pr || plot_roc)
    ...
    if (~hold_fig)
        ...
        if (plot_pr)
            ...
            plot([0; tpr], [1 ; prec], style); % add pseudo point
                                               % to complete curve
        ...
        if (plot_roc)
            ...
            plot([0; fpr], [0; tpr], style); % add pseudo point to
                                             % complete curve
        ...
        if (plot_pr)
            ...
            plot([0; tpr],[1 ; prec], style); % add pseudo point to
                                              % complete curve
        end
