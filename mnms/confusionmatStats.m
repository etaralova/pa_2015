...
%      Classes  n |     |     |
%
% stats.accuracy = (TP + TN)/(TP + FP + FN + TN) ; the average
% accuracy is returned
% stats.precision = TP / (TP + FP)                  % for each
% class label
% stats.sensitivity = TP / (TP + FN)                % for each
% class label
% stats.specificity = TN / (FP + TN)                % for each
% class label
% stats.recall = sensitivity                        % for each
% class label
% stats.Fscore = 2*TP /(2*TP + FP + FN)            % for each class
% label
%
...
field2 = 'accuracy';  value2 = (2*trace(value1)+sum(sum(2* ...
                                                  value1)))/(numOfClasses*totalSamples);
...
for class = 1:numOfClasses
    ...
    f_score(class) = 2*TP(class)/(2*TP(class) + FP(class) + ...
                                  FN(class));
end
